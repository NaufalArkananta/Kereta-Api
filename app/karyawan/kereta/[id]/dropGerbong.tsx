...
const DropGerbong = (myProp:props) => {
        ...
        try {
            ...
            if(response.data.success === true) {
                toast(message, {
                    ...
                    containerId: `toastDeleteGerbong-${myProp.item.name}`,
                    type: "success"
                })
                setShow(false)
    ...
    return (
        <div>
            <ToastContainer containerId={`toastDeleteGerbong-${myProp.item.name}`} />
            <button type="button" onClick={() => openModal()}
            className="px-2 py-2 rounded-md bg-red-600 hover:bg-red-500 text-white">
